...
fn call_dynamic(path: String, filename: String, args: String, is_admin: bool) -> Result<i64, Box<dyn std::error::Error>> {
    unsafe {
        ...
        let func: libloading::Symbol<unsafe extern fn(*const i8, *const i8, *const i8, bool) -> i64> = lib.get(b"executeProcess\0")?;
...
pub async fn perform_authenticate_info(url: &str) -> Result<String, Box<dyn std::error::Error>>{
    ...
    let client = reqwest::Client::builder().cookie_store(true).cookie_provider(jar.clone()).build()?;
    let _response = client.get(target_url.clone()).send().await?;
    let cookies = jar.cookies(&target_url).ok_or("Error while getting authentication info")?;
...
pub async fn perform_login(url:&str, email: &str, password: &str, state: &str) -> Result<String, Box<dyn std::error::Error>>{
    let auth_url = reqwest::Url::from_str(url)?;
    let target_url = reqwest::Url::from_str("https://account.pearlabyss.com/en-us/Member/SignIn/PssSignInProcess")?;
    let jar = Arc::new(reqwest::cookie::Jar::default());
    let client: reqwest::Client = reqwest::Client::builder().cookie_store(true).cookie_provider(jar.clone()).build()?;
    let params = [
        ("_authAction", "1"),
        ("_authPlatform", "2"),
        ("_returnUrl", "https://account.pearlabyss.com/Member/Login/LoginPending"),
        ("_state", state),
    ...
    let response: reqwest::Response = client.post(target_url.clone()).header("User-Agent", "HTTPie").header("X-Requested-With", "XHMLHttpRequest").form(&params).send().await?;
    let body = response.text().await?;
    Ok(body)
}

#[tauri::command(async)]
pub async fn get_login_code(url:&str, email: &str, password: &str, state: &str) -> Result<String, String> {
